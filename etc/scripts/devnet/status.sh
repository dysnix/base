...
L2_VALIDATOR_OP_RPC="${4:-$L2_VALIDATOR_OP_RPC_URL}"
...
# Fetch unified validator sync status
VALIDATOR_STATUS=$(curl -s $L2_VALIDATOR_OP_RPC -X POST -H "Content-Type: application/json" \
    --data '{"jsonrpc":"2.0","method":"optimism_syncStatus","params":[],"id":1}' 2>/dev/null)
VALIDATOR_UNSAFE=$(echo $VALIDATOR_STATUS | jq -r '.result.unsafe_l2.number // "N/A"')
VALIDATOR_SAFE=$(echo $VALIDATOR_STATUS | jq -r '.result.safe_l2.number // "N/A"')
...
printf "%-12s | %-10s | %-10s\n" "L2 Unified" "$VALIDATOR_UNSAFE" "$VALIDATOR_SAFE"
