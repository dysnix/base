...
pub(super) fn compare_nested_struct_type(
    ...
    let type_def = solc_layout
        ...
        .values()
        .find(|t| {
            // Extract type name after last dot (e.g., "struct B403Registry.PolicyData" -> "PolicyData")
            let type_name = t.label.rsplit('.').next().unwrap_or(&t.label);
            type_name == type_name_pattern
        })
        .ok_or_else(|| {
            vec![format!("Type '{}' not found in Solidity layout", type_name_pattern)]
        })?;
...
fn compare_type_members(
    ...
    let members = type_def.members.as_ref().ok_or_else(|| {
        vec![format!("Type '{}' does not have members (not a struct?)", type_def.label)]
    })?;
    ...
    for solc_member_name in solc_member_info.keys() {
        if !rust_member_fields.iter().any(|rm| rm.name == solc_member_name) {
            errors.push(format!(
