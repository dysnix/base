...
use base_precompiles::storage::{Mapping, Set, StorageCtx};

fn expect_members_match<T>(
    set: &mut base_precompiles::storage::SetHandler<T>,
    expected: &[T],
) -> eyre::Result<()>
where
    T: base_precompiles::storage::Storable
        + base_precompiles::storage::StorageKey
        + std::hash::Hash
        + Eq
        + Clone
        + std::fmt::Debug,
    T::Handler: base_precompiles::storage::Handler<T>,
{
    // Check length
    assert_eq!(
        set.len()?,
    ...
    let at_values: Vec<T> = (0..expected.len()).map(|i| set.at(i).unwrap().unwrap()).collect();
    for value in expected {
...
fn test_oz_starts_empty() -> eyre::Result<()> {
    ...
    StorageCtx::enter(&mut storage, || {
        let mut set = base_precompiles::storage::SetHandler::<Address>::new(U256::ZERO, address);
...
fn test_oz_at_returns_none_for_nonexistent_elements() -> eyre::Result<()> {
    ...
    StorageCtx::enter(&mut storage, || {
        let set = base_precompiles::storage::SetHandler::<Address>::new(U256::ZERO, address);
...
fn test_oz_values_full_and_paginated() -> eyre::Result<()> {
    ...
    StorageCtx::enter(&mut storage, || {
        let mut set = base_precompiles::storage::SetHandler::<U256>::new(U256::ZERO, address);
        ...
        for begin in 0..=4 {
            for end in 0..=4 {
                ...
                let expected: Vec<U256> =
                    values.iter().skip(begin).take(end.saturating_sub(begin)).cloned().collect();
...
fn test_set_with_b256() -> eyre::Result<()> {
    ...
    StorageCtx::enter(&mut storage, || {
        let mut set = base_precompiles::storage::SetHandler::<B256>::new(U256::ZERO, address);
