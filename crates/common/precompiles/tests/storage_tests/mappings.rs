...
use super::*;
use base_precompiles::storage::{Mapping, StorageCtx};
...
fn test_mapping() {
    ...
    StorageCtx::enter(&mut storage, || {
        ...
        layout.profile_mapping[test_address(10)].write(profile1.clone()).unwrap();
        layout.profile_mapping[test_address(20)].write(profile2.clone()).unwrap();

        // Verify all entries
        assert_eq!(layout.block_mapping[1u64].read().unwrap(), block1);
        assert_eq!(layout.block_mapping[2u64].read().unwrap(), block2);
        assert_eq!(layout.profile_mapping[test_address(10)].read().unwrap(), profile1);
        assert_eq!(layout.profile_mapping[test_address(20)].read().unwrap(), profile2);
        ...
        Ok::<(), base_precompiles::error::BasePrecompileError>(())
    })
    .unwrap();
}
