pub use IB403Registry::{
    IB403RegistryErrors as B403RegistryError, IB403RegistryEvents as B403RegistryEvent,
};

crate::sol! {
    #[derive(Debug, PartialEq, Eq)]
    #[sol(abi)]
    interface IB403Registry {
        // Enums
        enum PolicyType {
            WHITELIST,
            BLACKLIST,
...
impl B403RegistryError {
    /// Creates an error for unauthorized calls
    pub const fn unauthorized() -> Self {
        Self::Unauthorized(IB403Registry::Unauthorized {})
    }

    /// Creates an error for incompatible policy types
    pub const fn invalid_policy_type() -> Self {
        Self::InvalidPolicyType(IB403Registry::InvalidPolicyType {})
    }

    /// Creates an error for incompatible policy types
    pub const fn incompatible_policy_type() -> Self {
        Self::IncompatiblePolicyType(IB403Registry::IncompatiblePolicyType {})
    }

    /// Creates an error for non-existent policy
    pub const fn policy_not_found() -> Self {
        Self::PolicyNotFound(IB403Registry::PolicyNotFound {})
    }

    pub const fn policy_not_simple() -> Self {
        Self::PolicyNotSimple(IB403Registry::PolicyNotSimple {})
    }

    /// Virtual addresses are B1022 forwarding aliases and cannot be used as policy members.
    pub const fn virtual_address_not_allowed() -> Self {
        Self::VirtualAddressNotAllowed(IB403Registry::VirtualAddressNotAllowed {})
    }
}
