...
impl Service {
    ...
    fn inject_session_established(
        ...
        // Apply the table filter to incoming sessions. Without this check,
        // peers that connect inbound bypass the config.table_filter that is
        // applied to FINDNODE-discovered peers. This ensures all peers in the
        // routing table satisfy the same filter criteria regardless of how
        // the session was initiated.
        if !(self.config.table_filter)(&enr) {
            trace!(node = %node_id, "Ignoring session: ENR rejected by table_filter");
            return;
        }
