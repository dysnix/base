...
pub struct TestConfig {
    ...
    ///
    /// Defaults to `false`, in which case recipients are picked from the
    /// sender pool (existing behavior). Only affects payloads that honor the
    /// runner-supplied `to` address (`transfer`, `calldata`); payloads that
    /// override `to` internally (`erc20`, `precompile`, `osaka`,
    /// `uniswap_v2`/`uniswap_v3`) are unaffected.
    ///
    /// Recipients are not reclaimed: each gets at most a few wei from the
    /// `transfer` payload, and the gas cost to sweep them back exceeds the
    /// value. The seed-based derivation just means you *can* sweep later if
    /// you ever want to.
    #[serde(default)]
